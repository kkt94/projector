...
# Prefer torchcodec, but keep decord available for per-sample fallback.
...
if not HAS_TORCHCODEC and not HAS_DECORD:
    raise ImportError(
        "Neither torchcodec nor decord is available. Please install decord: pip install decord"
    )
...
class VideoTransform(ImageTransform):
    ...
    def load_video(self, video_path, max_frames=16, sampling_fps=1, s=None, e=None):
        """
        Loads a video from a given path and extracts frames based on specified parameters.
        ...
        if HAS_TORCHCODEC:
            try:
                return self._load_video_torchcodec(
                    video_path, max_frames, sampling_fps, s, e
                )
            except Exception as e:
                if HAS_DECORD:
                    logger.warning(
                        f"torchcodec failed for {video_path}: {e}. Retrying with decord."
                    )
                    try:
                        return self._load_video_decord(
                            video_path, max_frames, sampling_fps, s, e
                        )
                    except Exception as e2:
                        logger.error(
                            f"Error loading video {video_path} with decord fallback: {e2}"
                        )
                        return None
                logger.error(f"Error loading video {video_path} with torchcodec: {e}")
                return None

        if HAS_DECORD:
            try:
                return self._load_video_decord(video_path, max_frames, sampling_fps, s, e)
            except Exception as e:
                logger.error(f"Error loading video {video_path} with decord: {e}")
                return None

        logger.error(f"No video decoder available for {video_path}")
        return None

    def _load_video_torchcodec(self, video_path, max_frames, sampling_fps, s, e):
    ...
    def _load_video_decord(self, video_path, max_frames, sampling_fps, s, e):
        ...
        # Decord returns numpy array in (N, H, W, C) format
        frames = vr.get_batch(sample_pos).asnumpy()
        # Convert to torch tensor in (N, C, H, W) format
        all_frames = torch.from_numpy(frames).permute(0, 3, 1, 2)
