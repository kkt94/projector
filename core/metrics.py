...
def _wandb_api_available() -> bool:
    return (
        ...
        and hasattr(wandb, "log")
        and hasattr(wandb, "finish")
...
class MetricLogger:
    ...
    def open(self):
        ...
        if (
            _wandb_api_available()
            and self.args is not None
            and self.args.logging is not None
            ...
            logger.warning(
                "WandB logging was requested but `wandb` is unavailable or incompatible. "
    ...
    def log(self, metrics: Dict[str, Any]):
        ...
        if (
            ...
            if not getattr(self, "_wandb_warned", False):
                logger.info("WandB unavailable or disabled; metrics logged to JSONL only.")
...
class GPUMemoryMonitor:
    ...
    def get_peak_stats(self):
        ...
        try:
            ...
            # Some environments do not expose NVML/pynvml. Keep training alive.
            if not self._power_draw_warned:
                logger.info(
                    "GPU power draw unavailable (pynvml not installed); "
                    "power_draw=0 in metrics."
                )
                self._power_draw_warned = True
            power_draw = 0
