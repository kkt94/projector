...
class DistributedArgs:
    # Distributed backend to use for training loop.
    # "fsdp": existing FSDP path, "deepspeed": DeepSpeed engine path.
    backend: str = "fsdp"
    dp_shard: int = (
    ...
    # Optional DeepSpeed settings (used when backend="deepspeed")
    deepspeed_config: Optional[str] = None  # Path to DeepSpeed JSON config
    deepspeed_zero_stage: int = 2
    deepspeed_offload_optimizer: str = "none"  # "none" or "cpu"
    deepspeed_offload_param: str = "none"  # "none" or "cpu"
    deepspeed_steps_per_print: int = 2000
...
def is_deepspeed_enabled(distributed_args: DistributedArgs) -> bool:
    return distributed_args.backend.lower() == "deepspeed"
...
def parallelize_model(
    ...
    if distributed_args.full_activation_checkpointing:
        ...
        if hasattr(model, "vision_model"):
            # Check for CLIP-style architecture (transformer.resblocks)
            # V-JEPA2 and other encoders may have different structures
            if hasattr(model.vision_model, "transformer") and hasattr(model.vision_model.transformer, "resblocks"):
                for (
                    layer_id,
                    resblock,
