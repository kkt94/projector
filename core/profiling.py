...
try:
    ...
    xformers = None
    # Use dummy base classes so subclasses can be defined
...
def _wandb_profile_logging_enabled() -> bool:
    return (
        wandb is not None
        and hasattr(wandb, "Html")
...
class PyTorchProfilerWandb(PyTorchProfiler):
    ...
    def _on_trace(self, prof: torch.profiler.profiler.profile) -> None:
        ...
        if get_is_master() and _wandb_profile_logging_enabled():
            filename = list(
