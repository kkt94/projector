...
from core.distributed import (DistributedArgs, EnvironmentArgs,
                              ...
                              is_deepspeed_enabled, parallelize_model,
                              requeue_slurm_job, setup_env,
                              setup_torch_distributed)
from core.deepspeed_utils import (get_deepspeed_grad_norm, get_optimizer_lr,
...
def train(args: TrainArgs):
    with ExitStack() as context_stack:
        ...
        # Parallelize model
        if use_deepspeed:
            logger.info("Using DeepSpeed backend (FSDP sharding disabled)")
        else:
        ...
        if args.probe_freq is not None:
            if use_deepspeed:
                raise ValueError("probe_freq is not supported with DeepSpeed backend")
            if get_is_master():
        ...
        # Train loop
        log_model_params(
            model.module if use_deepspeed else model
