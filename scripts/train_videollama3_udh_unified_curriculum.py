...
try:
    ...
    FULL_TRAIN_DATASETS = UDH_CAPPED_TRAIN_DATASETS
...
class VideoLLaMA3FixedVideoQADataset(Dataset):
    ...
    def _preprocess_frames(self, frames):
        ...
        if tensor.shape[-2] != self.target_size or tensor.shape[-1] != self.target_size:
            # Match Videollama3ImageProcessor's PIL BICUBIC resize.
...
class VideoOnlyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Resize, to tensor, normalize. Match Videollama3ImageProcessor: PIL BICUBIC.
        processed = []
        for f in frames:
            f = f.resize((self.target_size, self.target_size), Image.BICUBIC)
...
def build_base_model(args, device, rank):
    ...
    # VideoLLaMA3VisionEncoderModel doesn't support flash_attention_2; when fa2 is requested
    # apply it to the LLM only and leave the vision encoder on sdpa.
    if args.attn_implementation == "flash_attention_2":
        from transformers import AutoConfig
        _cfg = AutoConfig.from_pretrained(model_path, trust_remote_code=True)
        _cfg._attn_implementation_internal = "flash_attention_2"
        if hasattr(_cfg, "vision_encoder_config"):
            _cfg.vision_encoder_config._attn_implementation_internal = "sdpa"
        base_model = AutoModelForCausalLM.from_pretrained(
            model_path, config=_cfg,
