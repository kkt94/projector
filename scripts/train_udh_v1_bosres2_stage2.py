"""
Train UDH v1 Stage 2 — Branch-Output Residual Scaler.

Uses the same inference form as CtxRes2:
  z_final = z_base + a_local(x) * local_residual + a_state(x) * state_residual

Key difference from earlier stage2 methods:
  - Target is derived from z_base vs z_temp (local+state combined), not z_local/z_state individually
  - The local/state split within the temporal budget is guided by branch norm ratio
  - QA gradient does NOT flow through the scaler (target-only supervision)

This avoids evaluating z_local or z_state in isolation, which Stage 1 never trained on.
"""
...
from core.vision_projector.dual_proj import RawContextResidualScaler, UnifiedDualHeadExpert
...
SUPPORTED_STAGE1_TYPES = {
    "dual_proj_deltanet_udh_v1_stage1": "dual_proj_deltanet_udh_v1_bosres2",
}
...
def train(args):
    ...
    log_rank0(rank, "=== UDH v1 Stage 2: Branch-Output Residual Scaler ===")
