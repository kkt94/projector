"""
Unified A/B/C training for InternVL3 + SplitDualHeadExpert + TemporalQueryRouter.
...
Replaces the 3-script approach (stage1 → stage2 → stage3).
"""
...
from train_internvl3_udh_unified_curriculum import (
    InternVL3VideoQADataset,
    VideoOnlyDataset,
    build_base_model,
    build_dataset,
    build_loader,
...
class UnifiedABCModel(nn.Module):
    ...
    def _build_vision_features(self, pixel_values):
        ...
        n_total = bsz * t
        chunk_size = self._vision_chunk_size if self._vision_chunk_size > 0 else n_total
        all_proj = []
        with torch.no_grad():
...
def save_abc_checkpoint(output_dir, tag, step, wrapped, optimizer, adapter_config, phase):
    ...
    if phase == "C" and bm is not None and hasattr(getattr(bm, "language_model", None), "save_pretrained"):
...
def train(args):
    ...
    log_rank0(rank, "=== Unified A/B/C Training ===")
    ...
    # ── Adapter config ──
    adapter_config = {
        "module_type": "internvl3_unified_abc",
    ...
    loader_a = build_loader(video_only_ds, batch_size=args.batch_size, sampler=sampler_a, shuffle=sampler_a is None, num_workers=args.num_workers, collate_fn=video_only_collate_fn)
    ...
    while global_step < args.phase_a_steps:
        ...
        with torch.amp.autocast("cuda", dtype=torch.bfloat16):
            loss = wrapped.forward_phase_a(pixel_values)
    ...
    n_total_lm = sum(p.numel() for p in base_model.language_model.parameters())
    ...
    # Reset _lm_inner cache since we just wrapped language_model with PeftModel
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Unified A/B/C training for InternVL3 + Expert + Router + LoRA")
    ...
    # torch.compile (unused but kept for compatibility with build_base_model)
