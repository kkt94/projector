"""
Train UDH v1 Stage 1 — Unified Dual Head Temporal Expert.

Single DeltaNet at full pre-pool resolution (1024 tokens), with two output heads:
  - local head: per-token gate × up → pool(1024→256)
  - state head: mean(spatial) → gate × up → broadcast to 256

Both aux losses supervise the same DeltaNet hidden:
  - L_local_corr: per-token correspondence prediction
  - L_state_mask: masked temporal state prediction from spatial-mean hidden
"""
...
from core.vision_projector.dual_proj import UnifiedDualHeadExpert
...
class PLMWithUDHExpert(nn.Module):
    ...
    def _masked_state_loss(self, aux):
        state_h = aux["state_h"]  # (B, T-1, 128) — spatial-mean of hidden_h
        ...
        # state_h is T-1 (from diffs), map to frame indices: state_h[i] corresponds to frame i+1
    ...
    def _local_correspondence_loss(self, z_pre_pool, aux):
        ...
        hidden_h = aux["hidden_h"].reshape(bt, s_pre, self.expert.bottleneck)
    ...
    def forward(self, input_ids, attention_mask, pixel_values_videos, labels=None, num_frames=32, global_step=0, **kwargs):
        ...
        z_pre_pool = z_pre_pool_flat.reshape(b, t, n_pre, d)
        ...
        local_corr_loss, local_corr_acc = self._local_correspondence_loss(z_pre_pool, aux) if self.local_corr_lambda > 0 else (
...
def train(args):
    ...
    log_rank0(rank, "=== UDH v1 Stage1: Unified Dual Head Temporal Expert ===")
    ...
    pooling_ratio = base_model.model.multi_modal_projector.pooling.pooling_ratio
    ...
    adapter_config = {
        "module_type": "dual_proj_deltanet_udh_v1_stage1",
        "version": "udh_v1_stage1",
        ...
        for batch in loader:
            ...
            if rank == 0 and global_step % args.log_interval == 0:
                ...
                print(
                    ...
                    f"diff_n={expert._last_diff_norm:.3f} hid_n={expert._last_hidden_norm:.3f} "
...
def build_parser():
    ...
    p.add_argument("--output_dir", type=str, default="outputs/udh_v1_stage1")
    ...
    p.add_argument("--local_corr_anchors", type=int, default=64)
